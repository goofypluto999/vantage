...
//
// Built 2026-05-11. Architecture-safe redo of the earlier attempt
// (commit 106a0bf, reverted). The server lives at action 'jobsearch'
// inside api/interview/[action].ts, with helpers in api/_lib/
// (Vercel-bundled per the post-incident pattern).
//
// All multi-agent-review fixes from the first attempt baked in:
//   - HIGH: anonymous redirect has distinct loading copy
//   - HIGH: match-score circle role="meter" + aria-valuenow/min/max
//   - HIGH: Apply-via-Vantage uses location.state to confirm pre-fill
//   - HIGH: loading stages mark complete on result arrival
//   - MED: token-balance preflight on Run-scan + dynamic CTA label
//   - MED: hero copy adapts after first scan (no "first free" lie)
//   - MED: 44×44 mobile touch targets on all card actions
//   - MED: ghost badge color tier (50-74 amber / 75+ rose)
//   - MED: Save-to-tracker derived from live tracker entries
//   - MED: source-not-configured banner when Adzuna keys missing
//   - MED: hours-to-free-reset surfaced in needsTopUp error
//   - MED: work-mode chips use aria-pressed only (no radiogroup mismatch)
//   - MED: aria-controls on expand toggle
//   - LOW: safeHref() rejects javascript:/data:/vbscript: schemes
...
export default function JobSearchPage() {
  ...
  // Tracker-derived saved-job keys (live from useApplicationTracker)
  ...
  async function handleSearch() {
    ...
    try {
      ...
      // Mark all stages complete on result arrival (review HIGH fix).
      ...
      <main id="main" className="max-w-6xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <label htmlFor="js-location" className="block text-xs font-semibold text-white/70 mb-1">
                <MapPin className="w-3.5 h-3.5 inline mr-1" aria-hidden="true" /> Location (city, region, postcode)
              </label>
        ...
        {meta.sourceReport && meta.sourceReport.adzuna?.state === 'not-configured' && (
          <div className="mb-6 p-3 rounded-lg bg-amber-500/10 border border-amber-500/30 text-amber-200 text-xs flex items-start gap-2">
            ...
            <p>Multi-country search (Adzuna) is not yet configured on this deploy. You're seeing global remote results only (via Remotive). UK/US/EU coverage coming online soon.</p>
          </div>
        )}
