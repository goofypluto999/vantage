...
function generatePlaybook(input: PlaybookInput): DayEntry[] {
  ...
  days.push({
    day: 4,
    title: 'CV refresh — strip AimVantage Lab acronyms, add metrics',
  ...
  days.push({
    day: 10,
    title: 'Apply to 5 roles. With AimVantage prep packs.',
    blurb: `Pick 5 roles from your 3 target companies. For each, run a AimVantage prep pack (90 seconds each = 8 minutes total) so the cover letter is tailored to the actual JD. Generic cover letters from a laid-off candidate read as desperate; tailored ones read as deliberate.`,
    tools: [
...
function buildMarkdown(input: PlaybookInput, days: DayEntry[]): string {
  ...
  lines.push('');
  lines.push(`Generated by AimVantage at https://aimvantage.uk/tools/layoff-playbook · ${COUNTRY_LABEL[input.country]} · ${input.severance === 'yes_3plus' ? '3+ months severance' : input.severance === 'yes_under3' ? '<3 months severance' : 'no severance'}.`);
  lines.push('');
  for (const d of days) {
          ...
          >
            ...
            <div className={`${t.glass} rounded-2xl p-8 text-center mt-6`}>
              ...
              <p className={`mt-2 ${t.textSub} max-w-xl mx-auto`}>
                ...
                letters. Manually that is 5 hours. With AimVantage that is 8 minutes. 10 free packs on
                signup, no card.
              </p>
              <Link
