...
import {
  Mail, AlertTriangle, ArrowRight, Copy, Check, Twitter, Linkedin, Loader2, Download,
} from 'lucide-react';
...
import { buildDecodeMarkdown, downloadMarkdown } from '../lib/exportMarkdown';
...
export default function DecodeRejectionPage() {
  ...
  // Last 5 decoded rejections, 30-day TTL. Same pattern as RoastPage.
  const { entries: history, push: pushHistory, remove: removeHistory, clear: clearHistory } =
    useResultHistory<DecodeResult>('vantage-decode-history-v1');
  ...
  const handleDecode = async () => {
    ...
    let data: any = null;
    try {
      // 30s client timeout. Server maxDuration is 20s — 10s headroom for
      // slow networks. Added 2026-05-11 to prevent stuck-loading UI.
      res = await fetchWithTimeout('/api/decode-rejection', {
        method: 'POST',
      ...
      if (!res.ok || !data) {
        ...
        setResult(decoded);
        const now = new Date();
        const label = `${decoded.verdict || 'decoded'} — sev ${decoded.severity ?? '?'}/5 — ${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}-${String(now.getDate()).padStart(2, '0')} ${String(now.getHours()).padStart(2, '0')}:${String(now.getMinutes()).padStart(2, '0')}:${String(now.getSeconds()).padStart(2, '0')}`;
        pushHistory({
          id: `decode-${now.getTime()}-${decoded.verdict || 'x'}`,
          label,
          result: decoded,
        });
      }
  ...
  const exportResult = () => {
    if (!result) return;
    const v = result.verdict || 'decoded';
    const ts = new Date().toISOString().slice(0, 19).replace(/[T:]/g, '-');
    downloadMarkdown(`vantage-decode-${v}-${ts}.md`, buildDecodeMarkdown(result));
  };
      ...
      <div className="max-w-3xl mx-auto px-6 py-12 md:py-20">
        ...
        <section className={`${t.glass} rounded-2xl p-6 md:p-8 mb-6`}>
          ...
          <div className="mt-4 flex flex-wrap items-center gap-2">
            ...
            >
              {loading ? <><Loader2 className="w-4 h-4 animate-spin" aria-hidden="true" /> Decoding…</> : <>Decode it <ArrowRight className="w-4 h-4" aria-hidden="true" /></>}
            </button>
        ...
        {/* Past results — same UX pattern as RoastPage. localStorage only,
            30-day TTL, last 5. Click to restore — no AI cost. */}
        {history.length > 0 && !result && !loading && (
          <div className={`${t.glass} rounded-2xl p-5 md:p-6 mb-6`}>
            <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
              <p className={`text-sm font-semibold ${t.text}`}>
                Past decodes <span className={`text-xs font-normal ${t.textMuted}`}>({history.length} saved on this device)</span>
              </p>
              ...
              {history.map((entry) => (
                <li key={entry.id} className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    aria-label={`Remove past decode: ${entry.label}`}
          ...
          <div className={`${t.glass} rounded-2xl p-6 md:p-8 mb-8`}>
            <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
              <div className={`flex items-center gap-2 text-xs uppercase tracking-wider ${t.textMuted}`}>
                <Mail className="w-3.5 h-3.5" aria-hidden="true" /> Decoded
              </div>
            ...
            <div className="flex flex-wrap gap-2 mb-6">
              ...
              >
                {copied ? <><Check className="w-3.5 h-3.5" aria-hidden="true" /> Copied</> : <><Copy className="w-3.5 h-3.5" aria-hidden="true" /> Copy decode</>}
              </button>
              <button
                type="button"
                onClick={exportResult}
                aria-label="Download this decode as a Markdown file for Notion or Obsidian"
