...
// Job-search source adapters. Imported via api/_lib/ — NOT a top-level
// /lib/ (which Vercel's bundler didn't include and crashed prod in the
// 106a0bf incident). The preflight check `api-no-escaping-imports`
// (903122e) blocks any future regression.
import {
  fetchAllSources, ADZUNA_COUNTRIES,
  type JobSearchParams, type WorkMode, type AdzunaCountry, type PostedWithinDays,
} from '../_lib/jobSources';
...
// AI Job Search constants. 1 token = pack of 10 curated jobs. First
// scan in any 24h window is free per signed-in user (refresh-proof via
// profiles.last_free_jobsearch_at). Anonymous users bounced at auth.
...
// Per-user in-process serial guard (multi-agent review MED). Stops
// a fast-double-click from firing two parallel scans before the
// deduct-tokens chain settles. Resets on cold start; acceptable.
 ...
 * `String({})` would otherwise ship "null" / "[object Object]" as text. */
...
// AI-curated multi-source job search. The "tool → service" milestone.
//
// All multi-agent-review fixes from the first attempt baked in:
//   - HIGH (sec): prompt-injection guard around third-party JD content
//     using <<<UNTRUSTED_JOB_LISTINGS_START>>> delimiters + server-
//     controlled [JOB#N|src=X] line prefixes
//   - HIGH (sec): fail-closed on NaN/unparseable last_free_jobsearch_at
//   - HIGH (sec): REVOKE UPDATE on profiles columns (in migration SQL)
//   - MED (sec): per-user serial guard (jobsearchInFlight)
//   - MED (sec): mock-source double-gated on NODE_ENV + JOBSEARCH_MOCK
//   - MED (sec): per-source state in response (configured/errored)
//   - MED (UX): hours-to-free-reset surfaced in 402 response
//   - graceful empty-results path with honest source-state messaging
async function handleJobSearch(request: any, response: any) {
  ...
  try {
    const body = request.body || {};
    const {
      keywords, location, country, workMode, salaryMin, postedWithin,
    } = body;
    ...
    // Fail-closed on NaN/unparseable timestamp (HIGH-sec fix).
    ...
    // ── AI score + curate (with prompt-injection guard) ──
    ...
    const rawJobLines = rawForAI.map((j, idx) => {
      const salaryStr = j.salaryMin || j.salaryMax
        ...
        : 'Salary: not listed';
      // Strip our own sentinel pattern from user-supplied text so a
      // hostile JD can't forge a [JOB#...] prefix. Cap to 600 chars.
...
For each of the top 10 ranked jobs (BY MATCH SCORE, descending), output a JSON object with:
  ...
  - "ghostProbability": integer 0..100 — clichés like "rockstar/ninja", suspiciously wide salary bands, multiple seniorities collapsed, no specific deliverables
...
- Output STRICT JSON array of exactly 10 objects (or fewer if fewer than 10 raw jobs were provided). No markdown fences. No prose.`;
    ...
    // Final-review LOW #18: if AI hallucinated all indexes outside the
    // valid range, `scored` is empty even though Adzuna/Remotive
    // returned real jobs. Don't burn the user's free scan or their
    // token on zero results — refund + skip the timestamp update.
    if (scored.length === 0) {
      if (didCharge) await refundTokens(user.id, JOBSEARCH_COST);
      return response.status(200).json({
        jobs: [],
        ...
        token_balance: newBalance,
        was_free: !didCharge,
        message: 'AI scoring returned no valid mappings. Tokens were not consumed. Try a slightly different keyword or location.',
      });
    }

    // Mark the free scan used (only AFTER AI success on free path).
    if (!didCharge) {
      try {
